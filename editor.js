const defaults = {
  ...
  brandX: 0, brandY: 0, brandScale: 100, statsX: 0, statsY: 0, statsScale: 100,
  utilitiesX: 0, utilitiesY: 0, utilitiesScale: 100, fishX: 0, fishY: 0, fishScale: 100, fishOpacity: 70,
...
const groups = {
  ...
  brand: { title: '主标题', fields: [['brandX', '水平位置', -600, 600], ['brandY', '垂直位置', -300, 300], ['brandScale', '尺寸', 45, 180, 1, '%']] },
  fish: { title: '鱼图', fields: [['fishX', '水平位置', -700, 700], ['fishY', '垂直位置', -360, 500], ['fishScale', '尺寸', 25, 220, 1, '%'], ['fishOpacity', '透明度', 5, 100, 1, '%']] },
  stats: { title: '有效物种与更新时间', fields: [['statsX', '水平位置', -600, 600], ['statsY', '垂直位置', -300, 300], ['statsScale', '尺寸', 45, 180, 1, '%']] },
  utilities: { title: '主题与语言按钮', fields: [['utilitiesX', '水平位置', -600, 600], ['utilitiesY', '垂直位置', -300, 300], ['utilitiesScale', '尺寸', 45, 180, 1, '%']] },
};
...
function customInspector(item) {
  const base = `${fieldCustomRange('x', '水平位置', -700, 700)}${fieldCustomRange('y', '垂直位置', -400, 600)}${fieldCustomRange('width', '宽度', 20, 900)}${fieldCustomRange('height', '高度', 20, 500)}${fieldCustomRange('opacity', '透明度', 0, 100, 1, '%')}${fieldText('colour', '主颜色', item.colour, 'color')}${fieldText('textColour', '文字颜色', item.textColour, 'color')}`;
...
function renderInspector() {
  ...
  const customActions = custom ? '<span><button id="raise-custom" type="button">上移</button> <button id="lower-custom" type="button">下移</button> <button id="remove-custom" type="button">删除</button></span>' : '';
  ...
  inspector.querySelector('#raise-custom')?.addEventListener('click', () => moveCustom(custom.id, 1));
  inspector.querySelector('#lower-custom')?.addEventListener('click', () => moveCustom(custom.id, -1));
}
...
function renderLayers() {
  const builtins = [['hero', '▦ 标题区'], ['brand', 'T 主标题'], ['fish', '◒ 鱼图'], ['stats', '▤ 统计卡片'], ['utilities', '◉ 主题/语言']];
  const custom = layout.customElements.map((item) => [`custom:${item.id}`, item.type === 'operation' ? `▣ ${item.text || '操作 UI'}` : `◇ ${item.text || '图形/文字元素'}`, item]);
...
function moveCustom(id, direction) {
  const from = layout.customElements.findIndex((item) => item.id === id); const to = from + direction;
  if (from < 0 || to < 0 || to >= layout.customElements.length) return;
  const reordered = [...layout.customElements]; [reordered[from], reordered[to]] = [reordered[to], reordered[from]]; layout.customElements = reordered; sync();
}
...
function addCustom(type) {
  ...
  const item = type === 'operation'
    ? { id, type, shape: 'rectangle', text: '新操作', action: 'scroll', url: '', x: 0, y: 0, width: 120, height: 40, fontSize: 15, colour: '#0c6570', textColour: '#ffffff', opacity: 100, radius: 10 }
    : { id, type: 'text', shape: 'rectangle', text: '新文字', action: 'scroll', url: '', x: 0, y: 0, width: 160, height: 36, fontSize: 18, colour: '#0c6570', textColour: '#0c6570', opacity: 100, radius: 10 };
